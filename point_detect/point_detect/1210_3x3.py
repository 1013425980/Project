...
def friction_calculate(img_gray, method='DYNAMIC_RANGE'):
    ...
    for i in range(i_start+1, i_end, i_step):
        for j in range(j_start+1, j_end, j_step):
            ...
            # max = np.max(window, axis=1)
            # min = np.min(window, axis=1)
            # friction[i-1,j-1] = np.mean(np.max(max) - np.min(min))

            # 中心值
            center_value = window[1, 1]

            # 其余值
            other_values = window[np.arange(3)[:, None], np.arange(3)]  # 除了中心值以外的所有值

            # 计算差值
            differences = other_values - center_value
            # 计算绝对值
            abs_differences = np.abs(differences)
            friction[i-1,j-1] = np.mean(abs_differences)

            # friction[i-1,j-1] = np.max(window) - np.min(window)
            # # 计算
            # if method == 'DYNAMIC_RANGE':
            #     friction[i-1,j-1] = np.max(window) - np.min(window)  # 动态范围表征
            # elif method == 'STD':
            #     friction[i-1,j-1]  = np.std(window)  # 标准差表征
...
def vociety_calculate(img_gray, friction, initial_velocity, acceleration, method='left'):
    ...
    # friction = friction_calculate(img_gray, kernel)
...
friction = cv2.GaussianBlur(friction, (15, 15), 0)
...
plt.subplot(133).imshow(friction, cmap='gray')
